...
def get_threshold(img,level=3):
    otsu = OtsuFastMultithreshold()
    otsu.load_image(img,bins=img.max())
    for k in 1+list(range(level)):
...
#     im = cv2.imread('/home/chenhao/device/method_test_save/grayAd/xiaoxi_307_bone.png',cv2.IMREAD_UNCHANGED)
#     otsu = OtsuFastMultithreshold()
#     otsu.load_image(im,bins=im.max())
#     for k in [1, 2, 3]: # 找出3个阈值来
